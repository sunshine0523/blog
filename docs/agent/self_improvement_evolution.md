...
- **工作方式**：
  ...
  - 使用**语义去重机制**（通过 embedding 相似度检查）避免重复内容
...
这种方式的优势在于**精确性**。它避免了两个常见问题：一是规则无限膨胀导致的上下文溢出，二是新规则覆盖旧规则导致的经验丢失。
...
- MCE 中，这个改进机制本身也是可以被优化的对象

Base Level 负责执行改进，Meta Level 负责优化改进策略。当 Meta Level 根据反馈调整了 CE Skill，Base Level 就会按照新的策略来改进 Context。这就实现了对**改进机制本身的优化**。
